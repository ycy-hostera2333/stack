...
class GrowthValue(Strategy):
    ...
    description = (
        ...
        "两者相关性 -0.15（负相关），成长与价值是真正对立的风格，"
        "组合是实打实的分散而非同一赌注押两次。\n"
        "基本面数据严格 point-in-time：只用已过法定披露截止日的报告。\n"
        "⚠ exit() 恒为 False，必须配「最长持有(日)」使用，建议 20（与因子测算的"
        "前瞻窗口一致）。不设的话是买满仓位后永不换手，而且不会报错。\n"
        "⚠ 2026-08 在修复后的数据上重跑（2019-2026，893 只经流动性过滤的池子）：\n"
        "  · 因子层：头部五分位超额 +0.35%/20日、逐年 6/8 为正（2019、2025 为负），"
        "Newey-West 修正 t 值 1.75。\n"
        "  · 策略层（20 日调仓）：年化 +16.3%、超额 +144.8%、胜率 50.6%，但逐年"
        "只跑赢基准 3/8 年——超额几乎全部来自 2022-2024，2019-2021 连续跑输。\n"
